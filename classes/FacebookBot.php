...
abstract class FacebookBot extends Bot {
    ...
    protected function _curl($aOpts) {
        ...
        curl_close($rCurl);
file_put_contents(__DIR__.'/output.html', $sResponse);
        return $sResponse;
    }
    ...
    protected function _getContent($sUrl, $sCookies, $iBack = 0, $iYear = 0) {
        ...
        if(preg_match_all('/\<div\s+class\=\"g\"\>\<a[^\>]+href\=\"([^\"]+)\"/', $sResponse, $aMatches)) {
            $aLinks = $aMatches[1];
            ...
            elseif($iBack > 0) {
                ...
                if(preg_match('/\<p\>([^\<]+)\<\/p\>/', $aPosts[$iIndex], $aMatch)) {
                    $aRes['text'] = strip_tags($aMatch[1]);
                }
...
file_put_contents(__DIR__.'/result.txt', print_r($aRes, true));
        if(empty($aRes['img'])) {
            return $this->sendMessage($this->getChatId($aJson), $aRes['text']);
        }
        else {
            return $this->sendPhoto($this->getChatId($aJson), file_get_contents($aRes['img']), $aRes['text']);
        }
    }
}
